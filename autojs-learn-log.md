...
3. 逻辑套路抽离？
   ...
   3. 操作控件。 操作失败的可能性，可能需要处理一下。
   4. 返回，或者 loop 前面的。

4. 稳定性问题？
   除了 3 中描述的，异常判断。有时候，页面会呈现一些不被期待的元素。这个时候，只能忽略处理了。采取整个链路失败的策略进行处理。脚本引擎杀掉脚本的方式。
5. 代码的结构
   使用，配置，对象，工作流，3 结构方式，来编写 autojs 代码。
   在处理这种自动化场景的时候。通常最容易想到的就是以过程式的思考方式，进行串联所有操作。但是，这种动作，写出来的代码，逻辑太长且不够清晰。最多，抽取几个常用函数。来增加代码的复用。然而，还是逻辑太长不够清晰。那么，这个时候，可以对象化思考。以 app 页面上面的元素为对象。比如，帖子对象，列表对象，appTab 对象。操作上面，抽象为工作流。各种工作流。然后，就是对工作流的控制。即配置对象。可以看到以，这 3 类编写的方式。既可以有效的区分每个代码的功能。且能使得整体任务更加清晰。
